  ...
  try {
    ...
    for (let pageIdx = 0; pageIdx < totalPages; pageIdx++) {
      ...
      if (pageIdx > 0) {
        ...
        // Add header for subsequent pages (optional - you can customize this)
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(10);
        doc.text(`Invoice ${billData.voucherNo} (Continued...)`, pageWidth / 2, y - 10, { align: 'center' });
        
        // Redraw table header
        doc.setFillColor(27, 145, 218);
        doc.rect(marginLeft, y - 5, pageWidth - marginLeft - marginRight, 7, 'F');
        doc.setTextColor(255, 255, 255);
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(9);
        ...
        headers.forEach((h, i) => {
          if (h === 'Qty' || h === 'Rate' || h === 'Amount' || h === 'MRP' || h === 'Tax') {
            doc.text(h, x + (colWidths[i] / 2), y - 0.5, { align: 'center' });
          } else {
            const offset = h === 'S.No' ? colWidths[i] / 2 : 2;
            const align = h === 'S.No' ? 'center' : 'left';
            doc.text(h, x + offset, y - 0.5, { align: align });
          }
          x += colWidths[i];
        });
      ...
      pageItems.forEach((item, localIdx) => {
        ...
        doc.text(String(globalIdx + 1), x + (colWidths[0] / 2), y + rowHeight / 2, { align: 'center' });
        x += colWidths[0];
      ...
      // For the last page, add summary and footer
