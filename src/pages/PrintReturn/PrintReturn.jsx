...
export const generateSalesReturnPDF = async ({
  ...
  try {
    ...
    doc.text('PURCHASE RETURN', pageWidth / 2, yPos + 8, { align: 'center' });
    ...
    doc.text('Return No:', marginLeft, yPos);
    doc.text(billDetails.billNo || 'N/A', detailsValueX, yPos);
    yPos += 5;
    
    doc.text('Return Date:', marginLeft, yPos);
    doc.text(formatDate(billDetails.billDate), detailsValueX, yPos);
    yPos += 7;    
    ...
    doc.text(billDetails.custName || 'N/A', rightColumnX, rightYPos);
    rightYPos += 4;
    doc.text(`Phone: ${billDetails.mobileNo || 'N/A'}`, rightColumnX, rightYPos);
    rightYPos += 4;
    doc.text(`GST No: ${billDetails.gstno || 'N/A'}`, rightColumnX, rightYPos);
    rightYPos += 4;
    ...
    const qrCodeDataURL = await QRCode.toDataURL(billDetails.billNo || 'N/A', {
      width: 150,
      margin: 1,
      color: {
        dark: '#000000',
    ...
    // Open in new window for reliable printing
    ...
    if (printWindow) {
      printWindow.onload = function() {
        ...
        }, 1000); // Increased delay for PDF loading
      };
      ...
      const checkClosed = setInterval(() => {
        if (printWindow.closed) {
          ...
          console.log('Print window closed, cleanup completed');
        }
      }, 500);
    } else {
