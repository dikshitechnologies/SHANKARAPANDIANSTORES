...
import { getCompCode, getUCode } from "../../utils/userUtils.jsx";
import { PopupScreenModal } from "../../components/PopupScreens.jsx";
import PrintReceipt from "../PrintReceipt/PrintReceipt.jsx";
import PrintSalesInvoice from "../PrintSalesInvoice/PrintSalesInvoice.jsx";
import PrintInvoiceEstimate from "../../pages/PrintInvoiceEstimate/PrintInvoiceEstimate";
const TABLE_FIELDS = [
  "barcode",
  "itemName",
  "stock",
...
const Estimate = () => {
  ...
  const formPermissions = useMemo(
    () => ({
      add: hasAddPermission(PERMISSION_CODES.ESTIMATE),
      edit: hasModifyPermission(PERMISSION_CODES.ESTIMATE),
      delete: hasDeletePermission(PERMISSION_CODES.ESTIMATE),
    }),
  ...
  const validateItemSelected = (rowIndex) => {
    ...
    if (!item.itemName || item.itemName.trim() === "") {
      ...
      setTimeout(() => {
        document
          .querySelector(`input[data-row="${rowIndex}"][data-field="itemName"]`)
          ?.focus();
      }, 0);
  ...
  const [rowDeleteConfirmationOpen, setRowDeleteConfirmationOpen] =
    useState(false);
  ...
  const fetchNextBillNo = useCallback(async () => {
    ...
    try {
      ...
      const endpoint =
        API_ENDPOINTS.ESTIMATE.getNextBillNo(compCode);

      const response = await axiosInstance.get(endpoint);

      if (response) {
        let nextBillNo = "";
        if (typeof response === "object" && response.voucherNo) {
          nextBillNo = response.voucherNo;
        } else if (typeof response === "string") {
          nextBillNo = response;
        } else if (response.data && typeof response.data === "string") {
          nextBillNo = response.data;
        } else if (response.data && response.data.voucherNo) {
          nextBillNo = response.data.voucherNo;
        }
  ...
  const totalTaxAmount = useMemo(() => {
    return items.reduce((sum, item) => {
      ...
      const taxAmt =
        typeof calc === "string" ? 0 : parseFloat(calc.taxAmount || 0);
  ...
  const fetchItems = useCallback(async (type = "FG") => {
    try {
      ...
      const endpoint = API_ENDPOINTS.SALES_INVOICE_ENDPOINTS.getItemsByType(
        type,
        1,
        1000000,
      );
      ...
      if (response && response.data) {
        ...
        if (itemsArray.length > 0) {
          ...
          console.log(
            `FG Items fetched (${formattedItems.length} items):`,
  ...
  const fetchSavedInvoices = useCallback(async (page = 1, search = "") => {
    try {
      ...
      const endpoint = API_ENDPOINTS.ESTIMATE.getBillList(
        compCode || "001",
        page,
        20,search
      );
      ...
      const formattedInvoices = invoiceData
        .map((invoice, index) => {
          const voucherNo =
            invoice.billNo || invoice.voucherNo || invoice.invoiceNo || "";
      ...
      if (search && search.trim() !== "") {
        ...
        finalResults = formattedInvoices.filter((invoice) => {
          return (
            invoice.voucherNo &&
            invoice.voucherNo.toString().toLowerCase().includes(searchLower)
          );
        });
      }
  ...
  const fetchInvoiceDetails = async (voucherNo) => {
    try {
      ...
      const endpoint =
        API_ENDPOINTS.ESTIMATE.getVoucherDetails(voucherNo);
      ...
      if (response && response.data) {
        ...
        setBillDetails((prev) => ({
          ...
          billDate: billDate,
          mobileNo:
            header.mobileNO || header.mobileNumber || header.mobileNo || "",
          type:
            header.selesType === "W" || header.salesType === "W"
              ? "Wholesale"
              : "Retail",
          salesman:
            header.salesmanName || header.sManName || header.salesman || "",
          salesmanCode:
            header.salesCode || header.sManCode || header.salesmanCode || "",
          custName: header.customerName || header.custName || "",
          custCode:
            header.customercode || header.customerCode || header.custCode || "",
          partyCode:
            header.customercode || header.customerCode || header.custCode || "",
        }));
        ...
        if (itemsArray.length > 0) {
          ...
          const formattedItems = itemsArray.map((item, index) => {
            const itemCode =
              ...
              "";
            const itemName =
              item.itemName || item.fitemNme || item.fItemName || "";
  ...
  const deleteInvoice = async (voucherNo) => {
    try {
      ...
      const endpoint = API_ENDPOINTS.ESTIMATE.deleteBillNumber(
        voucherNo,
        compCode || "001",
      );
      ...
      if (response && response.data) {
        ...
        if (success) {
          ...
          const errorMsg =
            response.data.message || response.data.error || "Delete failed";
  ...
  const getStockByItemName = async (itemCode) => {
    ...
    try {
      const response = await axiosInstance.get(
        API_ENDPOINTS.SALES_INVOICE_ENDPOINTS.getStockByItemName1(
          itemCode.trim(),
        ),
      );
  ...
  const getPurchaseStockDetailsByBarcode = async (barcode) => {
    ...
    try {
      const response = await axiosInstance.get(
        API_ENDPOINTS.SALES_INVOICE_ENDPOINTS.getPurchaseStockDetailsByBarcode(
          barcode.trim(),
        ),
      );
  ...
  const handleInvoiceSelect = async (selectedInvoice) => {
    ...
    try {
      ...
      if (invoiceData) {
        ...
        alert(
          `Could not load invoice details for ${selectedInvoice.voucherNo}`,
        );
      }
    } catch (error) {
      ...
      if (customerCode && companyCode) {
        ...
        try {
          const lastBillResp = await axiosInstance.get(
            API_ENDPOINTS.SALES_INVOICE_ENDPOINTS.getLastBillNoByCustomer(
              customerCode,
            ),
          );
  ...
  const openCustomerHistory = async (customerName) => {
    try {
      ...
      // Call ledger API
      const response = await axiosInstance.get(
        API_ENDPOINTS.sales_return.getLedger(
          customerCode,
          compCode,
          fromDate,
          toDate,
    ...
    try {
      ...
      if (type === "item" && search) {
        ...
        itemsData = itemsData.filter((item) => {
          ...
          return name.toString().trim().toLowerCase().includes(searchLower);
        });
      }
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "billNo" && isEditing) {
      alert(
        "Cannot change invoice number when editing. Please create a new invoice for a different number.",
      );
      return;
    }
  ...
  ===================================================== */
    const existingIndex = items.findIndex(
      (it, idx) =>
        idx !== currentRowIndex && it.barcode && it.barcode.trim() === barcode,
    );
    ...
    if (e.key === "Enter") {
      ...
      if (!hasItemName) {
        if (currentRowIndex === 0) {
          ...
          setTimeout(() => {
            document
              .querySelector(`input[data-row="0"][data-field="itemName"]`)
              ?.focus();
          }, 0);
          return;
        }
  ...
  const handleAddItem = async () => {
    ...
    try {
      ...
      const barcodeData = await getPurchaseStockDetailsByBarcode(
        billDetails.barcodeInput,
      );
      const selectedRate = getRateByType(barcodeData);
      if (barcodeData) {
        ...
        const existingItemInList = itemList.find(
          (item) =>
  ...
  const handleAddItemFromLocal = (itemData) => {
    ...
    if (existingItemIndex !== -1) {
      ...
      const calc = calculateAmount(
        newQty,
        existingItem.sRate || existingItem.rate,
        existingItem.tax,
      );
      const newAmount = typeof calc === "string" ? calc : calc.amount;
        ...
        if (item.id === id) {
          ...
          if (field === "qty" || field === "sRate" || field === "tax") {
            ...
            updatedItem.taxAmount =
              typeof calc === "string" ? "0.00" : calc.taxAmount;
          }
  ...
  const saveSalesInvoice = async () => {
    ...
    try {
      ...
      const validItems = items.filter(
        (item) =>
          item.itemName && item.itemName.trim() !== "" && Number(item.qty) > 0,
      );
        ...
        try {
          ...
          if (response) {
            if (typeof response === "object" && response.voucherNo) {
              ...
              latestVoucherNo = response.data.voucherNo;
            }
          }
        } catch (fetchErr) {
      ...
      const endpoint = isInsert
        ? API_ENDPOINTS.ESTIMATE.CREATE_SALES
        : API_ENDPOINTS.ESTIMATE.UPDATE_SALES;
  ...
  const handleSave = () => {
    ...
    const itemsWithZeroAmount = validItems.filter(
      (item) => parseFloat(item.amount || 0) === 0,
    );
    if (itemsWithZeroAmount.length > 0) {
      toast.warning(
        "Cannot save invoice with items having 0 amount. Please check item rates.",
        {
          autoClose: 3000,
        },
      );
  ...
  const buildBillData = (voucherNoParam) => {
    ...
    // Convert date from YYYY-MM-DD to DD-MM-YYYY for print component
    let formattedDate = billDetails.billDate || "";
    if (formattedDate && formattedDate.includes("-") && formattedDate.indexOf("-") === 4) {
    ...
    return {
      ...
      customercode: billDetails.custCode || "",
      customerName: billDetails.custName,
      customerMobile: billDetails.mobileNo || "",
      gstno: billDetails.gstno || "",
      Type: billDetails.type === "Wholesale" ? "W" : "R",
      ...
      items: validItems.map((item) => {
        ...
        // Calculate tax amount based on gstMode
        let taxrs = 0;
        if (tax > 0 && qty > 0 && rate > 0) {
          if (gstMode === "Inclusive") {
            const total = qty * rate;
            taxrs = (total * tax) / (100 + tax);
          } else {
            const base = qty * rate;
            taxrs = (base * tax) / 100;
          }
        ...
        return {
          ...
          mrp: item.mrp || "",
          tax: tax,
          taxrs: parseFloat(taxrs.toFixed(2)),
          rate: rate,
          qty: qty,
  ...
  const styles = {
    ...
    inlineInput: {
      ...
      fontWeight: TYPOGRAPHY.fontWeight.normal,
      lineHeight: TYPOGRAPHY.lineHeight.normal,
      paddingTop: screenSize.isMobile
        ? "5px"
        : screenSize.isTablet
          ? "6px"
          : "8px",
      paddingBottom: screenSize.isMobile
        ? "5px"
        : screenSize.isTablet
          ? "6px"
          : "8px",
      paddingLeft: screenSize.isMobile
        ? "6px"
        : screenSize.isTablet
          ? "8px"
          : "10px",
      paddingRight: screenSize.isMobile
        ? "6px"
        : screenSize.isTablet
          ? "8px"
          : "10px",
    ...
    tableContainer: {
      ...
      margin: screenSize.isMobile
        ? "6px"
        : screenSize.isTablet
          ? "10px"
          : "16px",
      marginTop: screenSize.isMobile
        ? "6px"
        : screenSize.isTablet
          ? "10px"
          : "16px",
      marginBottom: screenSize.isMobile
        ? "70px"
        : screenSize.isTablet
          ? "80px"
          : "90px",
  ...
  return (
    ...
    >
              ...
              <input
                ...
                onKeyDown={(e) => {
                  ...
                  if (e.key === "Enter") {
                    ...
                    if (
                      !billDetails.custName ||
                      billDetails.custName.trim() === ""
                    ) {
                ...
                >
                      ...
                      <input
                        ...
                        onKeyDown={(e) => {
                          ...
                          if (e.key === "Enter") {
                            ...
                            if (
                              !item.itemName ||
                              (!item.itemName.trim() && item.fromBarcode)
                            ) {
                  ...
                  <td style={{ ...styles.td, textAlign: "right" }}>
                    ...
                    <input
                      ...
                      data-field="mrp"
                      onChange={(e) =>
                        handleItemChange(item.id, "mrp", e.target.value)
                      }
                      onKeyDown={(e) => {
                        if (e.key === "Enter") {
                          e.preventDefault();
                          const uomElement = document.querySelector(
                      ...
                      onKeyDown={(e) => {
                        if (e.key === "Enter") {
                          ...
                          const taxInput = document.querySelector(
                            `input[data-row="${index}"][data-field="tax"]`,
                          );
                          if (taxInput) {
                            taxInput.focus();
                            return;
                  ...
                  <td style={{ ...styles.td, textAlign: "right" }}>
                    ...
                    <input
                      ...
                      data-field="sRate"
                      onChange={(e) =>
                        handleItemChange(item.id, "sRate", e.target.value)
                      }
                      onKeyDown={(e) => {
                        if (e.key === "Enter") {
                          e.preventDefault();
      ...
      <ConfirmationPopup
        ...
        title={
          saveConfirmationData?.isEditing
            ? "Confirm UPDATE Estimate"
      ...
      <PopupListSelector
        ...
        fetchItems={(page, search) =>
          fetchItemsForPopup(page, search, "customer")
        }
      ...
      <PopupListSelector
        ...
        fetchItems={(page, search) =>
          fetchItemsForPopup(page, search, "salesman")
        }
      ...
      <PopupListSelector
        ...
        fetchItems={(page, search) => fetchInvoicesForPopup(page, search)}
      ...
      <ConfirmationPopup
        ...
        message={
          rowToDelete ? `Do you want to clear?` : "Do you want to clear?"
        }
        confirmText="Yes"
        cancelText="No"
        type="danger"
