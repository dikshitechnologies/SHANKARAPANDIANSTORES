...
const [partyName, setPartyName] = useState("");
  ...
  const fetchParties = async (resetData = false) => {
    try {
      ...
      const response = await get(
        API_ENDPOINTS.LEDGER.PARTY_LIST(
          currentPage,
          partyPageSize,
          partySearchText,
        ),
  ...
  const handlePartySelect = (partyItem) => {
  setSelectedParty(partyItem);

  // ✅ Set actual values
  setParty(partyItem.fAcname);
  setPartyCode(partyItem.fCode);

  // ✅ VERY IMPORTANT — update display text
  setPartyDisplay(partyItem.fAcname);
  ...
  const handleCompanySelect = (selectedCompany) => {
    if (selectedCompany === "ALL") {
      ...
      const isSelected = tempSelectedCompanyCode.includes(
        selectedCompany.compCode,
      );
      if (isSelected) {
        setTempSelectedCompany(
          tempSelectedCompany.filter(
            (name) => name !== selectedCompany.compName,
          ),
        );
        setTempSelectedCompanyCode(
          tempSelectedCompanyCode.filter(
            (code) => code !== selectedCompany.compCode,
          ),
        );
      } else {
        setTempSelectedCompany([
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const response = await get(
        API_ENDPOINTS.LEDGER.GET_LEDGER(
          partyCode,
          compCodeParam,
          fromDate,
          toDate,
  ...
  const exportToExcel = () => {
    try {
      ...
      csvContent +=
        "S.No,Date,Name,Voucher No,Type,Cr/Dr,Bill No,Bill Date,Amount\n";
...
,"${row.name || ""}",${row.voucherNo || ""},${row.type || ""},${row.crDr || ""},${row.billNo || ""},${formatDateDDMMYYYY(row.billet)},${amount.toFixed(2)}\n`;
      ...
      link.setAttribute(
        "download",
        `Ledger_${partyDisplay}_${fromDate}_to_${toDate}.csv`,
      );
        ...
        >
          <div style={styles.balanceItem}>
            ...
            <span style={styles.balanceValue}>
              ₹
              {parseFloat(openingBalance || 0).toLocaleString("en-IN", {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2,
              })}
            </span>
          ...
          <div style={styles.balanceItem}>
            ...
            <span style={styles.balanceValue}>
              ₹
              {parseFloat(
                (closingBalance.credit || 0) - (closingBalance.debit || 0),
    ...
    >
      ...
      >
        {isLoading && allParties.length === 0 ? (
          ...
          <div style={styles.emptyText}>No parties found</div>
        ) : (
          <>
            {allParties.map((partyItem) => {
              const isSelected =
                tempSelectedPartyCode === partyItem.fCode;
        ...
        <div style={styles.popupOverlay} onClick={handleCompanyPopupClose}>
          <div style={styles.popupContent} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={styles.listContainer}>
              ...
              <div
                style={
                  tempSelectedCompanyCode.length === allCompanies.length &&
              ...
              {allCompanies.length === 0 ? (
                ...
                filteredCompanies.map((companyItem) => {
                  const isSelected = tempSelectedCompanyCode.includes(
                    companyItem.compCode,
                  );
                  return (
                    <div
                      key={companyItem.compCode}
                      style={
                        isSelected ? styles.selectedListItem : styles.listItem
                      }
